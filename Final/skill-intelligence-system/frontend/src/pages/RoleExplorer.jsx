...
export default function RoleExplorer() {
    ...
    const toggleExpand = (roleId) => {
        const newExpanded = expandedRole === roleId ? null : roleId;
        setExpandedRole(newExpanded);

        // Update Assistant Context
        if (newExpanded) {
            const role = recommendations.find(r => r.role_id === newExpanded);
            updateContext(role ? { id: role.role_id, title: role.role_title } : null);
        } else {
            updateContext(null);
        }
    };
